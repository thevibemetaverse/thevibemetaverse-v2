...
export function setMovingAnimationForContext(ctx, isMoving) {
  ...
  if (ctx.idleAnimAction && ctx.runAnimAction) {
    const iw = ctx.idleAnimAction.getEffectiveWeight();
    const rw = ctx.runAnimAction.getEffectiveWeight();
    // Stacked crossfades can leave both weights at 0; recover before blending again.
    if (iw + rw < 0.02) {
      ctx.idleAnimAction.reset();
      ctx.runAnimAction.reset();
      ctx.idleAnimAction.play();
      ctx.runAnimAction.play();
      ctx.idleAnimAction.setEffectiveWeight(isMoving ? 0 : 1);
      ctx.runAnimAction.setEffectiveWeight(isMoving ? 1 : 0);
